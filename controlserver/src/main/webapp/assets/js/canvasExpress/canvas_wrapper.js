...
cwPrototype.measureText = function (text) {
  this.applyContextProperties();
  return this.canvas.measureText(text);
};
cwPrototype.fillText = function (text, aX, aY, aWidth) {
  this.applyContextProperties();
  this.canvas.fillText(text, aX, aY, aWidth);
};
cwPrototype.createPattern = function (image, repeat) {
  this.canvas.createPattern(image, repeat);
};
